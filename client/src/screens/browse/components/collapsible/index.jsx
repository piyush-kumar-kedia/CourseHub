...
const Collapsible = ({ course, color, state }) => {
    ...
    const folderData = useSelector(
        (state) => state.fileBrowser.currentYearFolderStructure
    );
    const currCourseCode = useSelector(
        (state) => state.fileBrowser.currentCourseCode
    );
            ...
            if (t) {
                if (initial) {
                    ...
                    try {
                        ...
                        dispatch(
                            ChangeFolder(t.children?.[t.children.length - 1])
                        );
                        setInitial(false);
                    }
                }
